...
namespace Unity.Testing.Fakes.Tests
{
	[TestFixture]
...
public class C { public void M() { try { throw new System.Exception(""Hello world""); } catch (System.Exception ex) { throw; } } }",
		...
		private void AssertRecordedHookInjection(string codeToPatch, string testSource, int[] ints, int[] ints1, [CallerMemberName] string testName = null)
		{
			...
			{
				PrologInjectorVisitor.InjectFakes(assembly, hookedTestAssemblyPath, Assembly.GetExecutingAssembly().Location);
		...
		private void AssertHookInjection(string codeToPatch, Expression<Func<Type[], object[], object>> hookLambda, [CallerMemberName] string testName = null)
		{
			var hookFunc = hookLambda.Compile();
			AssertHookInjection(codeToPatch, testName, hookFunc, hookFunc(null ,null));
		}

		private void AssertHookInjection(string codeToPatch, LambdaExpression hookLambda, [CallerMemberName] string testName = null)
		{
		...
		private static void AssertHookInjection(string codeToPatch, string testName, Func<Type[], object[], object> hookFunc, object expectedValueFromHook, object[] args = null, Type[] genArgs= null)
		{
		...
		private static string OutputPathFor(string outputFolder, string assemblyNamePrefix, IEnumerable<string> assemblyReferences, IEnumerable<string> sources)
		{
		...
		private static void WriteToStream(Stream ms, IEnumerable<string> values)
		{
			if (values == null)
				return;
	...
	internal class LiteralExpressionValueExtractor : CSharpSyntaxVisitor<object>
	{
	...
	public class CastlePatchedInterceptorRegistry
	{
		...
		private static IDictionary<long, Func<Type[], object[], object>> registered = new Dictionary<long, Func<Type[], object[], object>>();
